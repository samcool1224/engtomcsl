...
examples/train.jsonl       2,000 generated pairs (1,076 ambiguous)
examples/test.jsonl        400 generated pairs (239 ambiguous)
tests/test_core.py         25/25 passing
```

## What runs in this environment (verified)

- Synthetic dataset generation (unambiguous + all four ambiguity kinds).
- Full pipeline on a **StubBackend** (gold-driven) → 400/400 user-facing correctness.
...
- **Dialogue-policy evaluation vs baselines** (the headline result):
  policy = 88% correct at 1.10 questions, between ASK_NONE (57%, 0 Q) and ASK_ALL (100%, 1.20 Q).
...
3. **Fine-tune** (conditional): run `examples/finetune_lora.py` on the 2,000-pair training set.
