...
VALUE_PHRASE = {
    "right_value":  "{a} in the right part of the image",
    "left_value":   "{a} in the left part of the image",
    "above_value":  "{a} in the top part of the image",
    "below_value":  "{a} in the bottom part of the image",
...
# spec sampling
# ---------------------------------------------------------------------------
def _sample_objects(rng: random.Random, n_new: int, n_existing: int) -> List[Obj]:
    objs = []
...
def _try_make_unambiguous(rng: random.Random) -> Optional[Tuple[Spec, List[str]]]:
    ...
    objs = _sample_objects(rng, n_new, n_existing)
    ...
    for _ in range(n_constraints):
        if len(ids) >= 2 and rng.random() < 0.8:
            ...
            if name in ("wider_value", "taller_value"):
                ...
                conj.append(Relation(name, [a], rng.randint(300, 700)))
...
def gen_ambiguous_reference(rng: random.Random, paraphrase: ParaphraseHook = None) -> Optional[Sample]:
    ...
    side = "left" if d == "cleft" else "right"
