...
def choice_structural_match(pred: Spec, gold_json: dict,
                            require_same_options: bool = True) -> bool:
    """User-facing correctness for AMBIGUOUS specs. Matches when:
      (1) same number of CHOICE nodes, each gold CHOICE paired to a pred CHOICE with the same
          `kind` and `span`;
      (2) geometric CHOICEs preserve the full reading set (priors, option ordering, and the
          exact magnitude of a vague offset are ignored — those are not user-visible);
    ...
    for g in gc:
        ...
        for i, p in enumerate(pc):
            ...
            if g.get("kind") == "offset":
                # The word "well" does not determine whether the larger gap is 150, 200,
                # or 300 per-mille.  Require the same relation/arguments and both a zero and
                # positive reading, but do not grade the arbitrary positive magnitude.
                def offset_shape(ch):
                    rels = [o["formula"] for o in ch["options"]
                            if o.get("formula") and o["formula"].get("node") == "rel"]
                    bases = {(r["name"], tuple(r["args"])) for r in rels}
                    vals = {r.get("const") for r in rels}
                    return bases, (0 in vals and any(isinstance(v, (int, float)) and v > 0 for v in vals))
                ok = offset_shape(g) == offset_shape(p)
            else:
