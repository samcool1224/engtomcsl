...
Usage:
    ...
    python examples/finetune_lora.py
        (the bundled train.jsonl now contains 2,000 balanced, learnable samples)
...
FT_INSTRUCTION = (
    ...
    "object word), emit a CHOICE node instead of guessing. Plain left/right/above/below are "
    "partial; only completely/fully use cleft/cright/cabove/cbelow. Horizontally aligned is "
    "yeq and vertically aligned is xeq. Comparisons against objects are binary; *_value is "
    "only for numeric or image thresholds. Copy the detected objects array exactly."
)
...
def main():
    ...
    # Optionally enlarge the bundled 2,000-pair training set further.
